...
def get_ocr_instance(model_version=None) -> PaddleOCR:
    ...
    # Models live next to this package (app/pipeline/models/{version}/inference);
    # override with OCR_MODELS_DIR env to keep heavy weights outside the repo if needed.
    pipeline_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # → app/pipeline
    models_root = os.getenv("OCR_MODELS_DIR") or os.path.join(pipeline_root, 'models')
    rec_model_dir = os.path.join(models_root, model_version, 'inference')
