...
class FormStatus(str, enum.Enum):
    ...
    extracted      = "extracted"       # Đã trích xuất — OCR xong, chờ kiểm tra viên
...
# status hiển thị phía user
class DisplayStatus(str, enum.Enum):
    draft     = "draft"      # nháp, chưa nộp
    submitted = "submitted"  # đã nộp + đang xử lý nội bộ (submitted..reviewed/gate_rejected/failed/overdue)
    returned  = "returned"   # đã trả kết quả

# status kết quả của từng field
...
class FormTemplate(Base):
    ...
    id           = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    form_type_id = Column(UUID(as_uuid=True), ForeignKey("form_types.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Form(Base):
    ...
    id           = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    org_id       = Column(UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="SET NULL"), nullable=True, index=True)  # gửi về ai (phường tiếp nhận)
    submit_by    = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)  # ai là người submit form
    form_type_id = Column(UUID(as_uuid=True), ForeignKey("form_types.id", ondelete="SET NULL"), nullable=True, index=True)  # loại đơn

    status     = Column(Enum(FormStatus), default=FormStatus.submitted, nullable=False, index=True)
    is_gate_rejected = Column(Boolean, default=False, nullable=False)  # đang bị chặn ở cổng (định danh/địa chỉ/sai người)
    reviewer_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"),
                         nullable=True, index=True)  # cán bộ đang giữ/soát hồ sơ (lock under_review)
    ...
    tamtru    = relationship("TamtruForm", back_populates="form", uselist=False, cascade="all, delete-orphan", lazy="noload")
    evidences = relationship("Evidence", back_populates="form", cascade="all, delete-orphan", lazy="noload", order_by="Evidence.created_at")
    results   = relationship("FormResult", back_populates="form", cascade="all, delete-orphan", lazy="noload", order_by="FormResult.label")
...
class TamtruForm(Base):
    ...
    id                 = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    form_id            = Column(UUID(as_uuid=True), ForeignKey("forms.id", ondelete="CASCADE"),        nullable=False, unique=True)
...
class Evidence(Base):
    ...
    warped_img  = Column(String(512), nullable=True) 
    created_at  = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class FormResult(Base):
    ...
    db_value        = Column(Text, nullable=True)                  # giá trị thật trong CSDL để cán bộ tham chiếu (kể cả field mềm)
    note            = Column(Text, nullable=True)                  # lý do verdict
    status          = Column(Enum(FormResultStatus), default=FormResultStatus.need_review,     nullable=False, index=True)
    created_at      = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    form     = relationship("Form", back_populates="results")
    confirms = relationship("ResultConfirm", back_populates="checkpoint", cascade="all, delete-orphan", lazy="noload", order_by="ResultConfirm.created_at")
...
class ResultConfirm(Base):
    ...
    id           = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    checkpoint_id = Column(UUID(as_uuid=True), ForeignKey("form_results.id", ondelete="CASCADE"), nullable=False, index=True)
    confirmed_by = Column(UUID(as_uuid=True), nullable=True)
    created_at   = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    final_status = Column(Enum(FormResultStatus.invalid, FormResultStatus.valid), nullable=False)
